...
#ifdef FEEDBACK
  //apply_stellar_feedback(All.Time, &Sp);   
  apply_stellar_feedback_treewalk(All.Time, &Sp);  // call Ezra's feedback model after star formation but before hydro
  //run_feedback(All.Time, FEEDBACK_SNII, &Sp);
  //run_feedback(All.Time, FEEDBACK_SNIa, &Sp);
